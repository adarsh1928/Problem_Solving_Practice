...
int main()
{
    ...
    cin >> n;
    int arrival[n], burst[n], wait[n], turn[n], queue[n], temp_burst[n], ft[n];
    bool complete[n];
    ...
    cout << "\nP.ID\tArrival Time\tBurst Time\tWait Time\tTurnAround Time\tFinishing time"
         << endl;
    for (int i = 0; i < n; i++)
    {
        cout << "P" << i + 1 << "\t\t" << arrival[i] << "\t\t"
             << burst[i] << "\t\t" << wait[i] << "\t\t" << turn[i] << "\t\t" << ft[i] << endl;
    }
    for (int i = 0; i < n; i++)
    {
        avgWait += wait[i];
